...
class CrmFacebookForm(models.Model):
    ...
    def get_fields(self):
        ...
        r = requests.get("https://graph.facebook.com/v2.12/" + self.facebook_form_id, params = {'access_token': self.access_token, 'fields': 'questions'}).json()
        if r.get('questions'):
...
class CrmLead(models.Model):
    ...
    def get_opportunity_name(self, vals, lead, form):
        if not vals.get('name'):
            ...
            vals['name'] = '%s - %s' % (form.page_id.fb_name, vals['contact_name'])
        return vals['name']
